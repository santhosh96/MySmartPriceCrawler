...
class MobileSpider(Spider):
	...
	start_urls = ['http://www.mysmartprice.com/mobile/pricelist/samsung-mobile-price-list-in-india.html']
	...
	def parse_phone(self, response):
		...
		if ('Display' in features):
			...
			if('Size (in inches)' in display):
				display_size = float(paramreturn(display, 'Size (in inches)'))
				if(type(display_size) == str and display_size != ""):
					display_size = re.findall(r'\d+.\d+', display_size)
		...
		if ('Storage' in features):
			...
			if(type(internal) == str and internal != ""):
				internal = int(internal.split('GB')[0])
